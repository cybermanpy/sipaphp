...
if($radio==1 && $filtro!="" && (int)$filtro){
	$sql1.=" WHERE pktipo = '".$filtro."' ";
}
if($radio==2 && $filtro!="" && $filtro){
	$sql1.=" WHERE UPPER(tiponame) LIKE UPPER('%".$filtro."%') ";
}
...
if($radio==1 && $filtro!="" && (int)$filtro){
	$sql.=" WHERE pktipo = '".$filtro."' ";
}
if($radio==2 && $filtro!="" && $filtro){
	$sql.=" WHERE UPPER(tiponame) LIKE UPPER('%".$filtro."%') ";
}
$sql.=" ORDER BY ".$orden." ".$type." LIMIT ".$pagtam." OFFSET ".$inicio;
$res=connector1($user,$pass,$sql);
$arrayhead=array('Tareas','PK','Descripción');
$arraydb=array('pktipo','pktipo','tiponame');
$title="TABLA TIPOS";
$del="frmdeltipos.php";
$frm="frminstipos.php";
$filtro1="PK";
