...
include'../includes/pagesviewdet.php';
...
<?php
$index="searcheq.php";
$table=" direcciones dir, marcas m, descripciones d, proveedores p, conservacion c, tipo_documentos td, estados e, org_fin o, tipos ti, fichas f, responsables r, ubicaciones u, (bienes b LEFT JOIN traslados t ON b.id_bien = t.bien_id and t.ultimo='si') ";
$array=array('b.id_bien','modelo','nro_serie','des_desc','des_marca','nom_resp','des_ubic','cod_pat_mh','codpatnew');
$sql1=select($table,$array);
$sql1.=" WHERE (b.id_marca=m.id_marca AND b.id_desc=d.id_desc AND b.id_prov=p.id_prov AND b.id_conserv=c.id_conserv AND b.id_tipo_doc=td.id_tipo_doc AND b.id_estado=e.id_estado AND b.id_org_fin=o.id_org_fin AND b.id_estado=1 AND ti.pktipo=d.fktipo AND f.id_bien=b.id_bien AND t.id_resp=r.id_resp AND r.fkubicacion=u.id_ubic AND  u.fkdir=dir.pkdir) ";
if($bien!=""){
...
$arrayhead=array('Bien','Usuario','Dpto','Serie','Código MH','Código MH','Marca','Tipo','Modelo','Ficha','Partes');
$arraydb=array('id_bien','nom_resp','des_ubic','nro_serie','cod_pat_mh','codpatnew','des_marca','des_desc','modelo','id_bien','id_bien');
$title="BUSQUEDA DE EQUIPOS";

//Codigo para buscar los datos en la ficha
$tableficha='bienes b, fichas f';
$arrayficha=array('proc_mar','proc_mod','mem_cap','disco_cap');
$arrayfichahead=array('Procesador:','Tipo:','Memoria','HDD:');
$arrayfichadb=array('proc_mar','proc_mod','mem_cap','disco_cap');
$whereficha =" WHERE b.id_bien=f.id_bien AND f.id_bien='";
/************************************************************/
...
page($res,$total,$pagtam,$page,$inicio,$arraydb,$arrayhead,$title,$index,$frm,$del,$orden,$type,$tableficha,$arrayficha,$arrayfichadb,$arrayfichahead,$whereficha,$tableparte,$arrayparte,$arraypartedb,$arraypartehead,$whereparte,$user,$pass,$show,$dist);
?>
</body>
</html>