...
<body>
<?php
$index="viewdirecciones.php";
$table="direcciones";
$array=array('pkdir','sigladir','namedir');
$sql1=select($table,$array);
if($radio==1 && $filtro!="" && (int)$filtro){
	$sql1.=" WHERE pkdir = '".$filtro."' ";
}
if($radio==2 && $filtro!="" && $filtro){
	$sql1.=" WHERE UPPER(sigladir) LIKE UPPER('%".$filtro."%') ";
}
...
if($radio==2 && $filtro!="" && $filtro){
	$sql.=" WHERE UPPER(sigladir) LIKE UPPER('%".$filtro."%') ";
}
$sql.=" ORDER BY ".$orden." ".$type." LIMIT ".$pagtam." OFFSET ".$inicio;
$res=connector1($user,$pass,$sql);
$arrayhead=array('Tareas','PK','Siglas','Descripción');
$arraydb=array('pkdir','pkdir','sigladir','namedir');
$title="TABLA DIRECCIONES";
$del="frmdeldirecciones.php";
$frm="frminsdirecciones.php";
$filtro1="PK";
$filtro2="Descripción";
$filtro3="";
$filtro4="";
