<?php
function page($res,$total,$pagetam,$page,$inicio,$arraydb,$arrayhead,$title,$index,$frm,$del,$orden,$type,$filtro1,$filtro2,$filtro3,$filtro4,$filtro5,$filtro6,$frmmove,$frmchart){
if($filtro3=="" && $filtro4==""){
  ?>
	...
	while($row=fetchArray($res)){
        ...
        <?php
				switch($row['id_desc']){
					...
					default;
						?>
            <a onClick="mostrar1('<?=$frmmove?>?pk=<?=$row[$arraydb[$idb]]?>');return false;" href=""><img class="img" src="<?=$move?>"></a>
            <?php
				}
				?>
        <a onClick="mostrar1('<?=$frm?>?pk=<?=$row[$arraydb[$idb]]?>');return false;" href=""><img class="img" src="<?=$pencil?>"></a>
        <a onClick="mostrar1('<?=$del?>?pk=<?=$row[$arraydb[$idb]]?>');return false;" href=""><img class="img" src="<?=$cross?>"></a>
        <?php
				switch($row['id_desc']){
					case 11:
						?>
            <a onClick="mostrar1('<?=$frmchart?>?pk=<?=$row[$arraydb[$idb]]?>');return false;" href=""><img class="img" src="<?=$chart?>"></a>
            <?php
					break;
					case 16:
						?>
