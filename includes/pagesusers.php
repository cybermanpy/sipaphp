<?php
function page($res,$total,$pagetam,$page,$inicio,$arraydb,$arrayhead,$title,$index,$frm,$del,$orden,$type,$lockadd,$lockbreak){
  ...
  $pencil="../img/16-em-pencil.png";
	$lock="../img/../img/lock_add.png";
	$lockb="../img/lock_break.png";
	...
	while($row=fetchArray($res)){
		...
		<?php
		 for($idb=0;$idb<count($arraydb);$idb++){
          ...
          <a onClick="mostrar1('<?=$lockadd?>?pk=<?=$row[$arraydb[$idb]]?>');return false;" href=""><img class="img" src="<?=$lock?>"></a>
          <a onClick="mostrar1('<?=$lockbreak?>?pk=<?=$row[$arraydb[$idb]]?>');return false;" href=""><img class="img" src="<?=$lockb?>"></a>          
        </td>
				<?php
			}else{
				?>
