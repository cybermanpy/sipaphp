<?php
function page($res,$total,$pagetam,$page,$inicio,$arraydb,$arrayhead,$title,$index,$frm,$del,$orden,$type,$tableparte,$arrayparte,$arraypartedb,$arraypartehead,$whereparte,$user,$pass,$delparte){
                  ...
                  for($ipartedb=0;$ipartedb<count($arraypartedb);$ipartedb++){
										switch($ipartedb){
											case 0;
												?>
                        <td>
                        <a onClick="mostrar1('<?=$delparte?>?fkform=<?=$rowparte[$arraypartedb[$ipartedb]]?>&fkbien=<?=$rowparte[$arraypartedb[$ipartedb+1]]?>');return false;" href=""><img class="img" src="<?=$cross?>"></a>
                        </td>
												<?php
											break;
											default;
