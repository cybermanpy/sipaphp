<?php
function page($res,$total,$pagetam,$page,$inicio,$arraydb,$arrayhead,$title,$index,$frm,$del,$orden,$type,$tableficha,$arrayficha,$arrayfichadb,$arrayfichahead,$whereficha,$tableparte,$arrayparte,$arraypartedb,$arraypartehead,$whereparte,$user,$pass,$show,$dist){
	...
	while($row=fetchArray($res)){
		...
		<?php
		 for($idb=0;$idb<count($arraydb);$idb++){
			 switch($idb){
				case (count($arraydb)-2):
					?>
					<td>
            <table class="tablereg2">
              <?php
              $sqlficha=select($tableficha,$arrayficha);
              $sqlficha.=" ".$whereficha." ".$row[$arraydb[$idb]]."' ";
              $resficha=connector1($user,$pass,$sqlficha);
              while($rowficha=fetchArray($resficha)){
                for($ificha=0;$ificha<count($arrayfichadb);$ificha++){
                  ?>
                  <tr>
                    <td><?=$arrayfichahead[$ificha]." ".$rowficha[$arrayfichadb[$ificha]]?></td>
                  </tr>
                  <?php					
                }
              }
